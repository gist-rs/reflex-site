...
//!   and the u8 standardizer equals the f64 one bit-for-bit;
//!   f64 one bit-for-bit;
...
pub const LANES_F: usize = 8;
pub const LANES_D: usize = LANES_F + 1;
...
mod wasm {
    ...
    use crate::{FLAPPY_D, FLAPPY_F, LANES_D, LANES_F, TETRIS_D, TETRIS_F};
    ...
    static LANES_BLOB: &[u8] = include_bytes!("lanes_corpus.bin");
    ...
    const LANES_N_OPTIONS: usize = 300;
    const LANES_N_STATES: usize = 100;
    static mut LANES_ROWS: [[f64; LANES_D]; LANES_N_OPTIONS] =
        [[0.0; LANES_D]; LANES_N_OPTIONS];
    static mut LANES_TARGETS: [f64; LANES_N_OPTIONS] = [0.0; LANES_N_OPTIONS];
    static mut LANES_OFFSETS: [u32; LANES_N_STATES + 1] = [0; LANES_N_STATES + 1];
    static mut LANES_ARGMAXES: [u8; LANES_N_STATES] = [0; LANES_N_STATES];
    static mut LANES_HEAD: [f64; LANES_D] = [0.0; LANES_D];
    static mut LANES_STD: Option<Standardizer<LANES_F>> = None;
    static mut LANES_OK: bool = false;
    static mut LANES_IN_AGREE: u32 = 0;
    static mut LANES_LAMBDA: f64 = f64::NAN;
    ...
    fn run_boot() {
        ...
        unsafe {
            ...
            let l = boot_one::<LANES_F, LANES_D>(
                LANES_BLOB,
                LANES_N_OPTIONS,
                LANES_N_STATES,
                &mut *addr_of_mut!(LANES_ROWS),
                &mut *addr_of_mut!(LANES_TARGETS),
                &mut *addr_of_mut!(LANES_OFFSETS),
                &mut *addr_of_mut!(LANES_ARGMAXES),
            );
            match l {
                Ok((lambda, agree, std, w)) => {
                    LANES_HEAD = w;
                    LANES_STD = Some(std);
                    LANES_IN_AGREE = agree;
                    LANES_LAMBDA = lambda;
                    LANES_OK = true;
                }
                Err(_) => LANES_OK = false,
            }
            BOOTED = true;
        }
    }
    ...
    pub extern "C" fn head_init() -> u32 {
        unsafe {
            ...
            // 0 = all heads ok; 1 = partial; 2 = none (the page reads the
            // mask for the truth).
            let m = head_ready();
            if m == 0b111 {
                0
    ...
    /// Ready MASK: bit0 = tetris, bit1 = flappy, bit2 = lanes.
    #[no_mangle]
    pub extern "C" fn head_ready() -> u32 {
        unsafe {
            (TETRIS_OK as u32) | ((FLAPPY_OK as u32) << 1) | ((LANES_OK as u32) << 2)
        }
    }
    ...
    #[no_mangle]
    pub extern "C" fn head_lanes_lambda() -> f64 {
        unsafe { LANES_LAMBDA }
    }

    #[no_mangle]
    pub extern "C" fn head_lanes_anchor() -> u32 {
        unsafe { LANES_IN_AGREE }
    }
    ...
    /// P(clean) for one lanes turn — the JOINED-STATE protocol: all three
    /// option sentences in pinned lane order plus the lane to score, or NaN
    /// when any sentence is off-grammar / names the wrong lane / lane ≥ 3.
    #[no_mangle]
    pub extern "C" fn head_score_lanes(
        p0: u32,
        l0: u32,
        p1: u32,
        l1: u32,
        ...
        unsafe {
            ...
            let std = match *addr_of!(LANES_STD) {
                Some(s) => s,
                None => return f64::NAN,
            };
            let read =
                |p: u32, l: u32| -> Option<&'static str> {
                    if l > (1 << 20) {
                        return None; // absurd length — refuse before the slice
                    }
                    let bytes = core::slice::from_raw_parts(p as *const u8, l as usize);
                    core::str::from_utf8(bytes).ok()
                };
            let (s0, s1, s2) = match (read(p0, l0), read(p1, l1), read(p2, l2)) {
