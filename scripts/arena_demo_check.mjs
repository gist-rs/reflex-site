...
// The Reflexer (rulebook search) walk (katgpt-rs Issue 892 hybrid FSM champion,
// recorded in-process by examples/tetris_09_site_walk.rs): the shared chain
// check, plus its own — the piece stream is the declared seed-607 stream
// regenerated by assets/games, argmax(ps) is the recorded pick, and the
// replayed score / lines / tetrises equal its _meta summary.
{
  const walk = oracle[RULEBOOK_WALK_KEY];
  const meta = oracle._meta?.sources?.[RULEBOOK_META_KEY];
  verifyTetrisWalk(walk, RULEBOOK_WALK_KEY);
  const r = verifyRulebookWalk(walk, meta);
  console.log(`${RULEBOOK_WALK_KEY}: stream = ${meta.stream.split(" — ")[0]} · replayed ${r.score} pts / ${r.lines} lines / ${r.tetrises} tetrises over ${r.pieces} pieces = _meta summary`);
}
